...
namespace Rush
{
    public class AchievementTaskItemUI : MonoBehaviour
    {
        [Header("Task Info")]
        ...
        private AchievementTaskConfig m_Task;

        // ── Setup ─────────────────────────────────────────────────────────────

        public void Setup(AchievementTaskState state)
        {
            m_Task = state.Config;
        ...
        public void Refresh(AchievementTaskState state)
        {
            RefreshProgressInternal(state);
            RefreshStateInternal(state);
        }
        ...
        private void RefreshRewardInternal(AchievementTaskConfig config)
        {
        ...
        private void RefreshProgressInternal(AchievementTaskState state)
        {
        ...
        private void RefreshStateInternal(AchievementTaskState state)
        {
        ...
        private void OnClaimClickedInternal() =>
            RushPlayer.Instance.AchievementManager.Claim(m_Task);
    }
}